...
#[test]
fn lint_and_fix_astro_template_expressions() {
    let fs = MemoryFileSystem::default();
    let mut console = BufferConsole::default();

    fs.insert(
        "biome.json".into(),
        r#"{ "html": { "experimentalFullSupportEnabled": true }, "linter": { "rules": { "style": { "useTemplate": "error" }, "nursery": { "useSortedClasses": "error" } } } }"#.as_bytes(),
    );
...
"#
    ...
    assert_cli_snapshot(SnapshotPayload::new(
        module_path!(),
        "lint_and_fix_astro_template_expressions",
        fs,
        console,
        result,
    ));
...
#[test]
fn check_and_fix_astro_template_expressions() {
    let fs = MemoryFileSystem::default();
    let mut console = BufferConsole::default();
...
"#
    ...
    assert_cli_snapshot(SnapshotPayload::new(
        module_path!(),
        "check_and_fix_astro_template_expressions",
        fs,
        console,
        result,
    ));
